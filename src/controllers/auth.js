...
exports.register = async (req, res) => {
  try {
    ...
    return res.status(201).cookie("token", token, { httpOnly: true,maxAge,sameSite: 'None', }).json({
      success: true,
...
exports.login = async (req, res) => {
  ...
  try {
    const token = await sign(payload, SECRET);
    const maxAge = 3600 * 1000;
    return res.status(200).cookie("token", token, { httpOnly: true,maxAge,sameSite: 'None' }).json({
      success: true,
